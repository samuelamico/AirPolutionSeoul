...
object Main extends App {
  ...
  val measureIndex = measureInfoDF.withColumn("id",monotonicallyIncreasingId)
  //measureIndex.select("Item code").where($"id" === 1).show()

  val result = measureIndex.select(col("Average value")).where($"id" === 1).collect()
